...
import BaseImgSlider from './BaseImgSlider.vue'
...
// Reactive mirror of the raw TYPO3 autoplay flag. BaseImgSlider parses this
...
<template>
  <BaseImgSlider :content="content" v-slot="{ uiProps }">
    <section
      :class="[
        'factory-img-slider relative w-full py-16 sm:py-20 lg:py-24',
        ...
        <div class="mt-10 lg:mt-12">
          <Splide ref="splideRef" :options="splideOptions(uiProps)" :aria-label="uiProps.title || 'Slider'">
            <SplideSlide v-for="(slide, i) in uiProps.slides" :key="slide._key">
              <figure class="factory-img-slider-slide">
                <div
...
.factory-img-slider :deep(.splide__pagination) {
  position: static;
  padding: 0;
  margin-top: 1.5rem;
  gap: 0.5rem;
}
.factory-img-slider :deep(.splide__pagination__page) {
  position: relative;
  overflow: hidden;
  width: 6px;
  height: 6px;
...
.factory-img-slider :deep(.splide__pagination__page.is-active) {
  width: 30px;
  background: var(--pagination-track);
}
.factory-img-slider :deep(.splide__pagination__page.is-active::after) {
  content: '';
  position: absolute;
  inset: 0;
...
.factory-img-slider.is-autoplay :deep(.splide__pagination__page.is-active::after) {
  transform: scaleX(0);
  animation: factory-slider-progress var(--slider-interval, 6s) linear forwards;
}
.factory-img-slider.is-paused :deep(.splide__pagination__page.is-active::after) {
...
.factory-img-slider :deep(.splide__arrow) {
...
.factory-img-slider :deep(.splide__arrow:hover) {
  background: rgba(0, 0, 0, 0.7);
}
.factory-img-slider :deep(.splide__arrow svg) {
  fill: currentColor;
}
</style>
