...
import BaseTeaser from './BaseTeaser.vue'
...
<template>
  <BaseTeaser :content="content" v-slot="{ uiProps }">
    <section class="factory-teaser relative w-full py-16 sm:py-20 lg:py-24">
...
.factory-teaser :deep(.splide__arrows) {
  position: absolute;
  right: 0;
  bottom: -3rem;
  display: flex;
...
.factory-teaser :deep(.splide__arrow) {
  position: static;
  transform: none;
  width: 2rem;
  height: 2rem;
...
.factory-teaser :deep(.splide__arrow:hover) {
  background: #f5f5f5;
}
.factory-teaser :deep(.splide__arrow:disabled) {
  opacity: 0.3;
  cursor: not-allowed;
  pointer-events: none;
}
.factory-teaser :deep(.splide__arrow svg) {
  fill: currentColor;
  width: 1rem;
  height: 1rem;
}
