...
// Reactive mirror of the raw TYPO3 autoplay flag. BaseHero parses this
...
<template>
  <BaseHero :content="content" v-slot="{ uiProps }">
    <section
      :class="[
        'factory-hero relative w-full',
...
.factory-hero :deep(.splide__pagination) {
  bottom: 1.25rem;
  gap: 0.5rem;
}
.factory-hero :deep(.splide__pagination__page) {
  position: relative;
  overflow: hidden;
  width: 6px;
  height: 6px;
...
.factory-hero :deep(.splide__pagination__page.is-active) {
  width: 30px;
  background: var(--pagination-track);
}
.factory-hero :deep(.splide__pagination__page.is-active::after) {
  content: '';
  position: absolute;
  inset: 0;
...
.factory-hero.is-autoplay :deep(.splide__pagination__page.is-active::after) {
  transform: scaleX(0);
  animation: factory-slider-progress var(--slider-interval, 6s) linear forwards;
}
.factory-hero.is-paused :deep(.splide__pagination__page.is-active::after) {
...
.factory-hero :deep(.splide__arrow) {
...
.factory-hero :deep(.splide__arrow:hover) {
  background: rgba(0, 0, 0, 0.6);
}
.factory-hero :deep(.splide__arrow svg) {
  fill: currentColor;
}
</style>
