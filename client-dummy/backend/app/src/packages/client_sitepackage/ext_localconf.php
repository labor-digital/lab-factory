...
(static function (): void {
    ...
    $discoverFactoryContentBlocks = static function (string $contentBlocksPath) use ($normalizeComponentKey, $readQualifiedName): array {
        ...
        foreach (new \DirectoryIterator($contentBlocksPath) as $fileInfo) {
            ...
            $components[] = [
                'key' => $componentKey,
                'qualifiedName' => $readQualifiedName($fileInfo->getPathname() . '/EditorInterface.yaml'),
            ];
        }
    ...
    $clientRootPath = dirname(__DIR__, 3);
    $factoryConfigPath = $clientRootPath . '/factory.json';
    $factoryContentBlocksPath = dirname(__DIR__, 4) . '/factory-core/typo3-extension/ContentBlocks/ContentElements';
    ...
    ExtensionManagementUtility::addPageTSConfig(implode(PHP_EOL, [
